...
def _load_classnames_and_classification_templates(
    ...
    DEFAULT_ZEROSHOT_CLASSIFICATION_TEMPLATES = zeroshot_classification_templates[
        "imagenet1k"
    ]
    ...
    if (
        ...
        name = dataset_name
    templates = zeroshot_classification_templates.get(
        name, DEFAULT_ZEROSHOT_CLASSIFICATION_TEMPLATES
    )
...
def build_dataset(
    ...
    **kwargs,
    ...
    if task in (
        ...
        classnames, templates = _load_classnames_and_classification_templates(
            dataset_name, current_folder, language
        )
    else:
    ...
    if dataset_name == "cifar10":
        ...
        annotation_file = os.path.join(
            root, multilingual_mscoco.CAPTIONS_FILE_NAME.format(language)
        )
        ...
        ds = BirdsnapV2(
            root=root, split="train" if train else "test", transform=transform, **kwargs
        )
...
def build_wds_dataset(
    ...
    if dataset_type == "retrieval":
        ...
        try:
            dataset.templates = [
                line.strip()
                for line in read_txt(templates_fname).splitlines()
                if line.strip()
            ]
        except FileNotFoundError:
