...
class Birdsnap(torch.utils.data.Dataset):
    ...
    def __init__(
        ...
        if not self.check_integrity():
            raise ValueError(
                "Dataset Birdsnap not downloaded completely or possibly corrupted."
            )
    ...
    def download(self):
        ...
        if self._check_integrity_of_metadata():
            ...
            download_and_extract_archive(
                self.METADATA_URL, self.root, filename=self.METADATA_ARCHIVE
            )
        # Actual files:
        self._parse_metadata()
    ...
    def _parse_metadata(self):
        ...
        self.paths = [
            os.path.join(self.root, self.BASE_FOLDER, entry["path"])
    ...
    def _verify_image(self, idx):
        try:
            ...
            with open(
                os.path.join(self.root, self.BASE_FOLDER, self.meta[idx]["path"]), "rb"
            ) as fin:
    ...
    def scrape_images(self, missing_ids, chunk_size=8196):
        ...
        def _load_url_and_save_image(idx, timeout):
            full_path = os.path.join(
                self.root, self.BASE_FOLDER, self.meta[idx]["path"]
            )
        ...
        with concurrent.futures.ThreadPoolExecutor(
            max_workers=None
        ) as executor:  # Choose max_workers dynamically
            # Start the load operations and mark each future with its URL
            future_to_url = {
                executor.submit(_load_url_and_save_image, idx, 600): self.meta[idx][
                    "url"
                ]
...
if __name__ == "__main__":
    ...
    dataset = BirdsnapV2(
        root="/mnt/petrelfs/wangwenhai/workspace/InternVL2/benchmark/imagenet/birdsnap",
        split="test",
    )
