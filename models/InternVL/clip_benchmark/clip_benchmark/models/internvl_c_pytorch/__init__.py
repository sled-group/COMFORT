...
class InternVLTokenizer(nn.Module):
    ...
    def forward(self, text, prefix="summarize:"):
        ...
        text = self.tokenizer(
            text, return_tensors="pt", max_length=80, truncation=True, padding=True
        ).input_ids
        return text


def build_transform(
    task, image_size=224, mean=[0.485, 0.456, 0.406], std=[0.229, 0.224, 0.225]
):
    if task == "retrieval":
        transform = T.Compose(
            [
...
def get_model_and_transform(task, image_size, device):
    ...
    model = InternVL_C(
        img_size=image_size, layerscale_force_fp32=True, llm_path=llm_path
    )
...
def load_internvl_c_pytorch(ckpt_path, device, task, image_size=224):
    ...
    model, transform = get_model_and_transform(
        task=task, image_size=image_size, device=device
    )
