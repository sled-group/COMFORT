...
def benchmark_model(model_name, benchmark_dir, device="cuda"):
    ...
    with open(csv_file, "r") as f:
        ...
        for i, row in tqdm(enumerate(reader)):
            ...
            img1 = (
                preprocess(images=img1, return_tensors="pt")
                .pixel_values.to(torch.float16)
                .cuda()
            )
            img2 = (
                preprocess(images=img2, return_tensors="pt")
                .pixel_values.to(torch.float16)
                .cuda()
            )
            ...
            with torch.no_grad():
                logits_per_image1, logits_per_text1 = model(
                    image=imgs, text=text1, mode=model_name
                )
                logits_per_image2, logits_per_text2 = model(
                    image=imgs, text=text2, mode=model_name
                )
