...
if __name__ == "__main__":
    ...
    for dataset in datasets:
        dataset_root = (
            ...
            else "/private/home/mitchellw/git/forks/CLIP_benchmark"
        )
                ...
                for k in ks:
                    ...
                    for lr in lrs:
                        for bs in batch_sizes:
                            pth = (
                                "/private/home/mitchellw/git/forks/CLIP_benchmark/probe_benchmark/data/"
                                + f"{model}-{pretrained}-{dataset}-{epochs}-{k}-{lr}-{bs}.json".replace(
                                    "/", "_"
                                )
                            )
                            print(pth)
                            ...
                            row = {
                                ...
                                "pretrained_short": (
                                    "laion2b" if "laion2b" in pretrained else pretrained
                                ),
                                "pretrained_clean": (
                                    "LAION" if "laion" in pretrained else "CLIP-WiT"
                                ),
                                "dataset": dataset,
                                "macts": compute_df[
                                    compute_df.model == model.replace("-quickgelu", "")
                                ...
                                "gmacs_total": mdf[
                                    mdf.model_fullname == models[ii].replace(",", " ")
                                ]["gmacs_total"].values[0],
                                "samples_seen": mdf[
                                    mdf.model_fullname == models[ii].replace(",", " ")
                                ]["samples_seen"].values[0],
                                "samples_seen_pretty": mdf[
                                    mdf.model_fullname == models[ii].replace(",", " ")
