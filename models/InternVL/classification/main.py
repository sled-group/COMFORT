...
def main(config):
        ...
        if data_loader_val is not None:
            if config.DATA.DATASET == "imagenet-real":
                ...
                acc1, acc5, loss = validate(
                    config, data_loader_val, model, amp_autocast=amp_autocast
                )
                logger.info(
    ...
    if config.TRAIN.EMA.ENABLE:
        ...
        if config.MODEL.RESUME:
            ...
            if config.DATA.DATASET == "imagenet-real":
                ...
                ), "imagenet-real should test with one gpu"
                filenames = dataset_val.filenames()
                filenames = [os.path.basename(item) for item in filenames]
                from dataset.imagenet_real import RealLabelsImagenet
                ...
                acc1, acc5, loss = validate(
                    config, data_loader_val, model_ema.ema, amp_autocast=amp_autocast
                )
                logger.info(
    ...
    for epoch in range(config.TRAIN.START_EPOCH, config.TRAIN.EPOCHS):
        ...
        if data_loader_val is not None and epoch % config.EVAL_FREQ == 0:
            acc1, acc5, loss = validate(
                config, data_loader_val, model, epoch, amp_autocast=amp_autocast
            )
            logger.info(
                f"Accuracy of the network on the {len(dataset_val)} test images: {acc1:.1f}%"
            )
...
def validate_real(config, data_loader, model, real_labels, amp_autocast=suppress):
    ...
    print(
        "* ReaL Acc@1 {:.3f} Acc@5 {:.3f} loss {losses:.3f}".format(
