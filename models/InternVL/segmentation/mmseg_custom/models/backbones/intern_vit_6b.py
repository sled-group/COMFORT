...
class LayerNorm(nn.Module):
    ...
    def forward(self, x):
        if self.data_format == "channels_last":
            ...
            x = self.weight[:, None, None].to(torch.float32) * x + self.bias[
                :, None, None
            ].to(torch.float32)
            x = x.to(input_dtype)
            return x
...
class InternViT6B(BaseModule):
    ...
    def __init__(
        ...
        use_flash_attn = (
            [use_flash_attn] * depth
            if not isinstance(use_flash_attn, list)
            else use_flash_attn
        )
    ...
    def init_weights(self, pretrained=None):
        ...
        def resize_pos_embed(pos_embed, H, W):
            ...
            pos_embed = (
                pos_embed[:, 1:, :]
                .reshape(1, self.pretrain_size // 14, self.pretrain_size // 14, -1)
                .permute(0, 3, 1, 2)
            )
            pos_embed = (
                F.interpolate(
