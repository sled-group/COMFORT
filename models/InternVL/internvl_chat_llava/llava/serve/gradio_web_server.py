    ...
    if len(state.messages) == state.offset + 2:
        ...
        if "llava" in model_name.lower():
            ...
            if any(x in temp_model_name for x in ["v1-1"]):
                ...
                x in temp_model_name
                for x in ["v1-2", "hermes2", "yi34b", "hermes-2", "yi-34b"]
            ):
...
def build_demo(embed_mode):
    ...
    with gr.Blocks(
        title="InternVL-Chat", theme=gr.themes.Default(), css=block_css
    ) as demo:
        state = gr.State()
        ...
        with gr.Row():
            with gr.Column(scale=3):
                ...
                gr.Examples(
                    examples=[
                        ...
                        [
                            f"{cur_dir}/examples/img3.jpg",
                            "Describe this image in detail",
                        ],
                        [
                            f"{cur_dir}/examples/img5.jpg",
                            "Please read the text in this image and return the information in the JSON format",
                        ],
                        [
                            f"{cur_dir}/examples/img6.jpg",
                            "How many dogs are in the figure, and why?",
                        ],
                    ],
            ...
            with gr.Column(scale=8):
                chatbot = gr.Chatbot(
                    elem_id="chatbot", label="InternVL-Chat", height=550
                )
                with gr.Row():
                    with gr.Column(scale=8):
                        textbox.render()
