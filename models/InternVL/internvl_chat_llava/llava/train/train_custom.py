...
class LazySupervisedDataset(Dataset):
    ...
    def __init__(
        ...
        if meta["annotation"].endswith("json"):
            ...
            self.list_data_dict = [
                json.loads(line) for line in open(meta["annotation"], "r")
            ]
            ...
            try:
                ...
                if "image" in sources[0]:
                    image_path = os.path.join(
                        self.root, self.list_data_dict[i]["image"]
                    )
...
def make_supervised_data_module(
    tokenizer: transformers.PreTrainedTokenizer, data_args, tcs_loader
) -> Dict:
...
def train(attn_implementation=None):
    ...
    data_module = make_supervised_data_module(
        tokenizer=tokenizer, data_args=data_args, tcs_loader=tcs_loader
    )
