...
class VQADataset(torch.utils.data.Dataset):
    ...
    def __init__(
        ...
        dirnames = [
            ...
            if os.path.exists(os.path.join(item, "dataset.json"))
        ]
        sorted(dirnames)
    ...
    def __getitem__(self, idx):
        root = self.roots[idx]
        item = self.items[idx]
        image_path, question, annotation = (
            item["image_path"],
            item["question"],
            item["gt_answers"],
        )
...
def evaluate_chat_model():
    ...
    for ds_name in args.datasets:
        ...
        for _, (pixel_values, questions, annotations, image_paths) in tqdm(
            ...
            for question, answer, annotation, image_path in zip(
                questions, answers, annotations, image_paths
            ):
        ...
        if torch.distributed.get_rank() == 0:
            ...
            cmd = (
                "python eval/tiny_lvlm/calculate_score.py "
