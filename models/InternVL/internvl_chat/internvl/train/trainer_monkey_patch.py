...
def get_num_layer_for_vit_and_qllama(var_name, vit_num_max_layer, llama_num_max_layer):
    ...
    if (
        var_name.startswith("clip_projector2.")
        or var_name.startswith("itm_head.")
        or var_name == "text_projection"
    ):
...
def create_optimizer(self):
    ...
    try:  # for stage2 model
        ...
        qllama_num_layers = (
            opt_model.internvl.config.qllama_config.num_hidden_layers + 2
        )
    ...
    for name, param in opt_model.named_parameters():
        ...
        layer_id = get_num_layer_for_vit_and_qllama(
            name, vit_num_layers, qllama_num_layers
        )
