...
class FlickrDataset(Dataset):
    ...
    def preprocess(self, image, caption, neg_caption):
        ...
        model_inputs["summarize_attention_mask"] = summarize_model_inputs[
            "attention_mask"
        ]
        ...
        content_attention_mask = tokenized_content["attention_mask"][
            :, 1:
        ]  # remove bos
        model_inputs["input_ids"] = torch.cat(
            [prefix_input_ids, content_input_ids], dim=1
        )
        model_inputs["attention_mask"] = torch.cat(
            [prefix_attention_mask, content_attention_mask], dim=1
        )
