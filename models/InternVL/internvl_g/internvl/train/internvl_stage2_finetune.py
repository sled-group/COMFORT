...
class ModelArguments:
    ...
    unfreeze_crossattn: bool = field(
        default=False,
        metadata={
            "help": "Set to True to unfreeze the cross attention layers in the QLLaMA."
        },
    )
    use_backbone_lora: int = field(
        default=0,
        metadata={
            "help": "If non-zero, indicates the use of LoRA in the vision backbone of the model"
        },
    )
    use_qllama_lora: int = field(
        default=0,
        metadata={
            "help": "If non-zero, indicates the use of LoRA in the QLLaMA of the model"
        },
    )
    ...
    drop_path_rate: float = field(
        default=0.0,
        metadata={
            "help": "Specify the value of drop path rate in the vision backbone. Default is 0."
        },
    )
...
def main():
    ...
    tokenizer = LlamaTokenizer.from_pretrained(
        model_args.model_name_or_path, add_eos_token=True
    )

    if "flickr" in data_args.dataset_name:
        train_dataset = FlickrDataset(
            metas=ds_collections[data_args.dataset_name],
            tokenizer=tokenizer,
            data_args=data_args,
        )
    ...
    model = InternVLModel.from_pretrained(
        model_args.model_name_or_path, ignore_mismatched_sizes=True, config=config
    )
