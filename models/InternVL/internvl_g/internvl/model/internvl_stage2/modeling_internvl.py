...
class InternVLModel(InternVLPreTrainedModel):
    ...
    def __init__(self, config: InternVLConfig):
        ...
        self.text_projection = nn.Parameter(
            torch.empty(text_hidden_size, clip_embed_dim)
        )  # frozen
    ...
    def _prepare_attention_mask(
        ...
        itc_mask[:, :, : self.num_query_token, self.num_query_token :] = torch.finfo(
            input_embeds.dtype
        ).min
        itc_mask[:, :, self.num_query_token :, : self.num_query_token] = torch.finfo(
            input_embeds.dtype
        ...
        itc_mask[
            :, :, self.num_query_token :, self.num_query_token :
        ] += itc_mask_causal
    ...
    def forward(
        ...
        input_ids = torch.cat(
            [
                negative_input_ids[: -self.positive_num],
                positive_input_ids[-self.positive_num :],
                summarize_input_ids,
                input_ids,
            ],
            dim=0,
        ...
        itm_attention_mask = torch.cat(
            [
                negative_attention_mask[: -self.positive_num],
                positive_attention_mask[-self.positive_num :],
            ],
            dim=0,
        )
        selected = itm_attention_mask.sum(1) - 1
        attention_mask = torch.cat(
            [itm_attention_mask, summarize_attention_mask, attention_mask], dim=0
        )  # [3 * batch_size, seq_len]
        ...
        neg_match_acc = (
            (logits[: batch_size - self.positive_num].argmax(dim=-1) == 0)
            / (batch_size - self.positive_num)
        ).sum()
        pos_match_acc = (
            (logits[-self.positive_num :].argmax(dim=-1) == 1) / self.positive_num
        ).sum()
        ...
        targets = torch.linspace(
            rank * bs, rank * bs + bs - 1, bs, dtype=torch.long, device=sim_i2t.device
        )
        ...
        if dist.get_rank() == 0:
            print(
                f"loss: {loss.item()}, loss_itm: {loss_itm.item()}, loss_itc: {loss_itc.item()}, "
