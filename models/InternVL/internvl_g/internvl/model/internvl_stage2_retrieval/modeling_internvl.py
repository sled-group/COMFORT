...
class InternVLModel(InternVLPreTrainedModel):
    ...
    def _prepare_attention_mask(
        ...
        itm_mask_neg, itm_mask_pos, itc_mask = torch.chunk(
            expand_mask, repeat_time, dim=0
        )
    ...
    def forward(
        ...
        input_ids = torch.cat(
            [negative_input_ids, positive_input_ids, summarize_input_ids], dim=0
        )  # [3 * batch_size, seq_len]
        itm_attention_mask = torch.cat(
            [negative_attention_mask, positive_attention_mask], dim=0
        )
        attention_mask = torch.cat(
            [itm_attention_mask, summarize_attention_mask], dim=0
        )  # [3 * batch_size, seq_len]
        ...
        loss_backbone_t2i = -torch.sum(
            F.log_softmax(backbone_t2i, dim=1) * sim_targets, dim=1
        ).mean()
        loss_backbone_i2t = -torch.sum(
            F.log_softmax(backbone_i2t, dim=1) * sim_targets, dim=1
        ).mean()
        loss_itc = (loss_t2i + loss_i2t) / 2 + (
            loss_backbone_t2i + loss_backbone_i2t
        ) / 2
